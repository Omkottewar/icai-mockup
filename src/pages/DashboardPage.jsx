...
import {
  IconAward, IconShield, IconCalendar, IconFileText, IconBookOpen, IconUsers,
      ...
      <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: '1fr', alignItems: 'start' }} data-dash-header>
        <div>
          ...
          <p className="muted-text" style={{ marginTop: '.15rem', fontSize: '.875rem' }}>
            {isMember ? 'Membership No.' : 'SRO No.'}{' '}
            <strong style={{ color: 'var(--foreground)' }}>{user.memberNo}</strong> · Member since {user.since}
          </p>
        </div>
              ...
              {MY_EVENTS.map((e) => (
                <li key={e.title} className="row" style={{ justifyContent: 'space-between', padding: '.75rem 0', borderBottom: '1px solid var(--border)', gap: '1rem' }}>
                  ...
                  <span className="badge" style={{
                    ...
                    color: e.status === 'Registered' ? 'var(--secondary)' : 'var(--accent-foreground)',
                  }}>{e.status}</span>
                </li>
              ))}
            </ul>
